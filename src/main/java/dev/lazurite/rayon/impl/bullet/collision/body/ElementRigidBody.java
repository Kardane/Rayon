...
public abstract class ElementRigidBody extends MinecraftRigidBody {
    ...
    private volatile ServerBodySnapshot serverSnapshot = new ServerBodySnapshot(
            new AABB(0, 0, 0, 0, 0, 0), 0.0f, 0.0f, false, false
    );
    private volatile PhysicsState persistenceSnapshot;
    ...
    public ElementRigidBody(PhysicsElement element, MinecraftSpace space, MinecraftShape shape, float mass, float dragCoefficient, float friction, float restitution) {
        ...
        this.refreshPersistenceSnapshot();
    }
    ...
    public void updateBoundingBox() {
        ...
        this.serverSnapshot = new ServerBodySnapshot(
                this.currentMinecraftBoundingBox,
                getSquaredSpeed(),
                getMass(),
                isActive(),
                terrainLoadingEnabled()
        );
    }
    ...
    public ServerBodySnapshot getServerSnapshot() {
        return serverSnapshot;
    }

    public PhysicsState getPersistenceSnapshot() {
        return persistenceSnapshot;
    }

    public void refreshPersistenceSnapshot() {
        this.persistenceSnapshot = PhysicsState.capture(this);
    }
    ...
    public record ServerBodySnapshot(AABB bounds, float speedSquared, float mass, boolean active,
